...
using namespace std;
vector<cJSON *> offlineQue;

void checkOffline(int recvID) {
    printf("进入offlienQue\n");
    printf("size : %d epty ? %d\n", offlineQue.size(), offlineQue.empty());
    for(int i = 0; i < offlineQue.size() && !offlineQue.empty(); i++) {
        printf("进入循环\n");
        cJSON *data = offlineQue.at(i);
        if(cJSON_GetObjectItem(data, "recvID")->valueint) {
            printf("找到关于%d的离线包\n");
            addSendQue(data);
            offlineQue.erase(offlineQue.begin() + i);
        }
    }
    printf("退出checkoffline\n");
}

void addOffline(cJSON *root) {
    offlineQue.push_back(root);
}
