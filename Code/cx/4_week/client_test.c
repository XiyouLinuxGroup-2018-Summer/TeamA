...
/*void add_usr(int sockfd)
{
    cJSON *json = cJSON_CreateObject();
    cJSON_AddNumberToObject(json, "signal", ADD_USR);
    int id;
    printf("please enter the id that you want to add\n");
    scanf("%d", &id);
    char buf[30];
...
void menu_select(int sockfd, int id)
{
    pthread_t thid1, thid2;
        ...
        printf("\t\t\t*********10.get_group_list*******\t\t\t\n");
        printf("\t\t\t*********11.get_usr_group_list*******\t\t\t\n");
        printf("\t\t\t*********12.delete group member**********\t\t\t\n");
