...
int Recursive(char *path,char c,char b);
...
int main (int argc, char * argv[])
{
	...
	else
	{
		...
		{
				Recursive(*(argv+1),'c','a');
				printf("m");
				return 0;
		}
		if(strcmp(argv[2],"-alR") == 0)
		{
				Recursive(*(argv+1),'l','a');
				return 0;
		}
			switch(m)
			{
...
int Recursive(char *path,char c,char b)
{
	...
	/*	if(*(p-1) == '.' )
		...
		if((strcmp(ptr->d_name,".")==0) || (strcmp(ptr->d_name,"..")==0))
		continue;
		if(Recursive(name,c,b) == 0)	//继续沿着目录文件向下访问
			printf("\n");
	}
	p[-1] = 0;
	if((m =closedir(dir)) == -1)
