/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2018年08月23日 星期四 14时20分51秒
 ************************************************************************/
...
//创建离线队列
queue<cJSON *> fuck_w; //背下,初始化队列

queue<cJSON *> t_file; //临时存储文件队列
...
#define SF_FR 18   //发文件
//#define SF_FR_R 19 //发文件回馈消息
#define R_SF_FR_R 20 //得到用户是否享受文件
...
void *analyse(void *wokao)
{
	...
	case SF_FR:
		...
		break;
	case R_SF_FR_R:
		sf_friend_r(&ff, user, ((dcan *)wokao)->connfd);
		break;
	case ZH_GR:
		zhuce_gr(&ff, user, ((dcan *)wokao)->connfd);
		break;
	}
}

//创建群函数
void zhuce_gr(smg *ff, cJSON *user, int connfd)
{
	char g_name[30];
	int g_number = cJSON_GetObjectItem(user, "g_number")->valueint;
	int g_zhu = cJSON_GetObjectItem(user, "g_zhu")->valueint;
	...
	char qql[512];
	sprintf(qql, "INSERT INTO group_list(g_number,g_name) VALUES(\'%d\',\'%s\');", g_number, g_name);
	if (0 != mysql_real_query(mysql, qql, strlen(qql)))
	{
		printf("query: %s\n", mysql_error(mysql));
	}
	...
	{
		if (atoi(row[1]) == g_number)
			g_id = atoi(row[0]);
	}
	mysql_free_result(shit);

	char sql[512];
	sprintf(sql, "INSERT INTO user_group(user_id,g_id,state) VALUES(\'%d\',\'%d\','1');", g_zhu, g_id);
...
//收文件回复函数
//回复加好友成功与否函数
void sf_friend_r(smg *ff, cJSON *user, int connfd)
{
	int beifa_fd;
	int beijia_id;
	int ifolag = 0;

	int key = cJSON_GetObjectItem(user, "send_f_r")->valueint;
	printf("key为%d", key);
	int beifa_id = cJSON_GetObjectItem(user, "beifa_id")->valueint;
	char *jia_name;
	...
	if (key == 1)
	{
		...
		{
			if ((beifa_id == atoi(tow[0])) && (atoi(tow[4]) == 1))
			{
				printf("iffolag已执行\n");
				ifolag = 1;
			}
		...
		if (ifolag == 1)
		{
			...
			for (int i = 0; i < fnum; i++)
			{
				...
				int jie_id = cJSON_GetObjectItem(tmp, "jie_id")->valueint;
				if (jie_id == beifa_id)
				{
					printf("一切正常\n");
					//pk_s(tmp, connfd);
					...
					printf("已进入\n");
					char *tmp_out = cJSON_Print(tmp);
					tmp_out = cJSON_PrintUnformatted(tmp); // 生成适合传输的JSON字符串
					//printf("发送的数据包是%s\n",tmp_out);
					...
					*(int *)package = number;
					...
					send(connfd, package, number + 4, 0);
					free(package);
					continue;
				}
				t_file.push(tmp);
...
void send_file_sr(smg *ff, cJSON *user, int connfd)
{
	...
	beifa_id = cJSON_GetObjectItem(user, "fr_id")->valueint;

	int get_connfd;
	//根据接受id向对象用户发送聊天数据
	static char ha[] = "SELECT * FROM user_list;";
	if (0 != mysql_real_query(mysql, ha, strlen(ha)))
	{
		printf("query: %s\n", mysql_error(mysql));
	}
	...
	cJSON_AddNumberToObject(rmessage, "jie_id", beifa_id); //我的id 我是想加好友的那个人

	cJSON *fangpi = cJSON_CreateObject();
	cJSON_AddNumberToObject(fangpi, "type", SF_FR_R);
	cJSON_AddNumberToObject(fangpi, "path", TS_PATH);
	cJSON_AddNumberToObject(fangpi, "beifa_id", beifa_id);
	cJSON_AddStringToObject(fangpi, "fa_name", fa_name);

	cJSON_AddNumberToObject(fangpi, "jie_id", beifa_id);

	printf("get_connfd是%d\n", get_connfd);

	if (key == 1)
	{
		t_file.push(rmessage);
		pk_s(fangpi, get_connfd);
	}
	else
	{
		//char *tmp_out= cJSON_PrintUnformatted(rmessage);// 生成适合传输的JSON字符串
		fuck_w.push(fangpi);
		t_file.push(rmessage);
	}
}
...
void chat_gr_sr(smg *ff, cJSON *user, int connfd, int mask)
{
	...
	{
		if (atoi(bow[2]) == ((*ff).si_ID) && (atoi(bow[1]) != wo_id))
		{
...
void add_friend_r(smg *ff, cJSON *user, int connfd)
{
	...
	if (key == 1)
	{
		...
		char bql[512];
		sprintf(bql, "INSERT INTO friend(user_id,friend_id) VALUES(\'%d\',\'%d\');", beijia_id, jia_ID);

		printf("这是拼接完成字符串%s\n", bql);

		if (0 != mysql_real_query(mysql, bql, strlen(bql)))
		{
			printf("query: %s\n", mysql_error(mysql));
		}
		mysql_free_result(kao);
...
void add_friend(smg *ff, cJSON *user, int connfd)
{
	...
	strcpy((*ff).user_name, cJSON_GetObjectItem(user, "wo_name")->valuestring);
	//printf("已执行\n");
	int get_connfd;
	//根据接受id向对象用户发送聊天数据
	char ha[] = "SELECT * FROM user_list;";
...
void zhuce_sr(smg *ff, cJSON *user, int connfd)
{
	...
	cJSON_AddNumberToObject(rmessage, "path", ZC_PATH);
	pk_s(rmessage, connfd);
	//printf("已发送 发送端口为%d\n",connfd);
}
	...
	while ((row = mysql_fetch_row(shit)))
	{
		if ((strcmp(row[1], ac) == 0) && (strcmp(row[2], pa) == 0))
		{
			...
			*user_ID = atoi(row[0]); //获取用户唯一主键
			...
			sprintf(result1, "update user_list set state=1 where ac_number=\"%s\";", row[1]);
			mysql_real_query(mysql, result1, strlen(result1));

			//获得登陆者fd
			sprintf(result2, "update user_list set duankou=%d where ac_number=\"%s\";", connfd, row[1]);
