...
#define SF_FR_R 25		//向服务器回复是否想要接收文件
...
void cha_menu(rmg *ff, cJSON *user, int fd)
{
	...
	do
	{
		...
		for (int i = 0; i < mxb; i++)
		{
			if (atoi(m_group[i][2]) == 1)
			{
				if (atoi(m_group[i][3]) == 1)
...
void add_gr_re(rmg *ff, cJSON *user, int fd)
{
	...
	printf("\t\t用户名为%s的用户请求加入 %s 群，您是否同意?\n", (*ff).f_name, g_name);
	printf("\n\n\n\t\tY[同意]       |       N[拒绝]\n");
...
void group_menue(rmg *ff, cJSON *user, int fd)
{
	...
	do
	{
		...
		printf("\n\n\n\n\n\n\n\n");
		printf("\n\n\n\n");
		printf("[S]创建群 |[A]加群 | [B]退群 | [C]群聊 |[N]收消息界面 | [E]查询群历史记录 | [D]解散群 |[F]查看群成员 |[Q] 退出\n");
...
void friend_menue(rmg *ff, cJSON *user, int fd)
{
	...
	{
		system("clear");
		printf("\t\t\t  用户主键ID               名字                      在线状态 \n");
		for (int i = 0; i < fxb; i++)
		{
			if (atoi(f_friend[i][2]) == 0)
				printf("\t\t\t  %7s                 %4s                        离线\n\n", f_friend[i][0], f_friend[i][1]);
			else
				printf("\t\t\t  %7s                 %4s                        在线\n\n", f_friend[i][0], f_friend[i][1]);
		}
		printf("\n\n\n\n\n\n\n\n\n\n");
...
void add_friend(int fr_id, int fd) //添加好友函数
{
	...
	cJSON_AddStringToObject(smessage, "ac_number", ac_number);
	cJSON_AddStringToObject(smessage, "wo_name", zhen_name);
	pk_s(smessage, fd);
...
void send_file(int fd)
{
	...
	while (tmpnumber != 0)
	{ //neitmpn=tmpn;
		tmpn = read(fp, tmpefile + f_size-tmpnumber, tmpnumber);
		printf("\t\t\t读了%d字节\n", tmpn);
		tmpnumber -= tmpn;
	...
	char *tmp_dout = cJSON_PrintUnformatted(smessage);
	printf("发送前的json长度为:%d\n", strlen(tmp_dout));
...
//发送文件信息回馈函数
void send_file_re(rmg *ff, cJSON *user, int fd)
{
	now_path = TS_PATH;
	//printf("已进入\n");
	strcpy((*ff).f_name, cJSON_GetObjectItem(user, "fa_name")->valuestring);
	//printf("已执行\n");
	printf("已执行\n");
	int beifa_id = cJSON_GetObjectItem(user, "beifa_id")->valueint;

	printf("\n用户名为%s的用户想向您传输文件，请选择\n", (*ff).f_name);
	printf("\n\n\nY[同意]   |   N[拒绝]\n");
	char ch;
	...
	cJSON *smessage = cJSON_CreateObject();
	cJSON_AddNumberToObject(smessage, "type", R_SF_FR_R);
	cJSON_AddNumberToObject(smessage, "beifa_id", beifa_id);
	cJSON_AddStringToObject(smessage, "user_name", (*ff).f_name);

	//cJSO_AddNumberToObject(smessage, "jia_ID", jia_ID);

	if (ch == 'y' || ch == 'Y')
	{
		cJSON_AddNumberToObject(smessage, "send_f_r", 1);
	}
	else if (ch == 'n' || ch == 'N')
	{
		cJSON_AddNumberToObject(smessage, "send_f_r", 0);
	}
	pk_s(smessage, fd);
}
...
void zhuce_qun(int fd)
{
	...
	cJSON_AddNumberToObject(smessage, "type", ZH_GR);
	cJSON_AddNumberToObject(smessage, "g_number", g_number);
	cJSON_AddNumberToObject(smessage, "g_zhu", user_ID);
	cJSON_AddStringToObject(smessage, "g_name", g_name);
