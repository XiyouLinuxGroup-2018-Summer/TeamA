...
{
	char *name;
	strcpy((*ff).f_id,cJSON_GetObjectItem(user,"f_id")->valuestring);
	strcpy((*ff).f_name,cJSON_GetObjectItem(user,"f_name")->valuestring);
	strcpy((*ff).f_state,cJSON_GetObjectItem(user,"f_state")->valuestring);

	printf("id= %s     name= %s    state= %s\n",(*ff).f_id,(*ff).f_name,(*ff).f_state);
