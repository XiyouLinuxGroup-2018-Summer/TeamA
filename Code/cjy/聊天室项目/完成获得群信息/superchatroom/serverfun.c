...
void group_f5_sr(smg *ff, cJSON *user, int connfd)
{
	...
	fetch_groupstate_byID(num, &xb, connfd,state);
}


//通过自身id获取群数量 顺便获取群数组
int fetch_group_byID(int user_ID, int *num, int *xb,char **state) //获得xb数量
{
...
//通过id获取群号与群名称
int fetch_groupstate_byID(int *num, int *xb,int connfd,char **state)
{
	int i = 0;
